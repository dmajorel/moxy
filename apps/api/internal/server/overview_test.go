...
func TestOverviewServesSnapshot(t *testing.T) {
	...
	newRouter(src).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/overview", nil))
...
func TestOverviewRejectsOtherMethods(t *testing.T) {
	rec := httptest.NewRecorder()
	src := fakeSource{overview: sampleOverview()}
	newRouter(src).ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/api/overview", nil))
...
func TestOverviewWithoutSource(t *testing.T) {
	rec := httptest.NewRecorder()
	newRouter(nil).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/overview", nil))
