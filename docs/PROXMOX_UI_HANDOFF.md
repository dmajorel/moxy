...
| Bouton maintenance | Fond ambre, icône `tool`, libellé « Mettre en maintenance ». Sous la liste des VM du nœud, rappel « Migration à la maintenance : automatique (HA) ». *(amendement : le bouton ouvre le **plan**, en lecture seule, et non une exécution — voir l'amendement du §4 ; le rappel n'est pas affiché tel quel, le plan disant invité par invité ce que le CRM déplacerait de lui-même.)* *(ré-amendement du 2026-09-17 : l'exécution revient, par SSH et non par l'API PVE. Le bouton ouvre toujours le **plan** — c'est sous le plan que la validation se fait, pas à sa place. → ADR 0010.)* |
| Confirmation maintenance | **Pas** de « Êtes-vous sûr ? » abstrait. Modal listant chaque VM → nœud de destination + RAM, un check de capacité **calculé avant le clic** (« 2202 passe à 34 % de RAM »), deux options : migrer aussi les VM hors HA, redémarrer le nœud une fois vide. Bouton de validation ambre foncé. Les templates restent sur place. *(amendement : les deux options et le bouton de validation sont des commandes d'exécution, et il n'y a pas d'exécution — voir l'amendement du §4. La modale rend le plan, signale les conteneurs qui seront redémarrés faute de migration à chaud, donne la commande `ha-manager` et les `qm migrate` / `pct migrate` des invités hors HA, puis se ferme. Les templates restent bien sur place.)* *(ré-amendement du 2026-09-17 : le bouton de validation ambre foncé reprend sa place, sous le plan — qui reste la confirmation, toujours pas de « Êtes-vous sûr ? » abstrait. La commande `ha-manager` reste affichée : c'est le recours quand l'exécution échoue. Les deux options du texte d'origine, elles, restent hors périmètre. → ADR 0010.)* |
...
| Icônes | Tabler Icons (`ti ti-*`). Libellés en français, sentence case. L'interface est depuis devenue bilingue : le français reste la langue de référence des maquettes et la source du catalogue, le *sentence case* vaut pour les deux langues. → ADR 0008. |
...
- **Maintenance de nœud** : disponible depuis PVE 7.x via HA.
  ...
  *(ré-amendement du 2026-09-17 : le constat ci-dessus tient — aucune route REST de maintenance n'est apparue depuis —, c'est la conclusion qui change. moxy exécute la commande CRM **par SSH**, sur un nœud du cluster autre que la cible, sous un compte sans shell atteignable et une grammaire de deux verbes. Il y aura donc un bouton d'exécution, et le SSH est un second canal : le token PVE reste en lecture seule. Décision, portée et modèle de menace dans l'[ADR 0010](adr/0010-node-maintenance-over-ssh.md), qui remplace l'ADR 0003.)*
...
4. Maintenance : endpoint `/api/clusters/{c}/nodes/{n}/maintenance/plan` (calcule le plan +
   ...
   *(ré-amendement du 2026-09-17 : l'exécution existe, mais ce n'est toujours pas
   `/execute` : c'est un `POST` sur `…/nodes/{n}/maintenance`, qui ouvre une session
   SSH au lieu d'appeler PVE. `plan` reste en lecture seule, et la commande
   `ha-manager` reste affichée comme recours. → ADR 0010.)*
